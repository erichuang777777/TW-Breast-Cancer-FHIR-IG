...
| 資源 | 數量 | 說明 |
|---|---|---|
| `CodeSystem` | 41 | 每個已驗證碼表一個。**display 用碼冊中文原文**，另附 `en` designation（本工具的英文臨床意義），`definition` 為中英合併 |
| `ValueSet` | 41 | 供 Questionnaire item 或 `Observation.valueCodeableConcept` 綁定 |
| `ConceptMap` | 41 | TCR 碼 → 標準術語的**骨架**：每個碼都列出來但 target 一律 `unmatched` |
...
| 類別 | 欄位數 | 狀態 |
|---|---|---|
| 已有驗證碼表（SSF1–10、結構欄位、腫瘤特性五欄、治療十欄、放射治療七欄、微創手術） | 41 | ✅ 有 CodeSystem/ValueSet，Questionnaire 綁定 |
| 純數值／日期／識別碼 | 20 | ✅ 型別為 integer/date/string，不需碼表 |
| 尚未轉錄碼表 | 38 | ⚠️ Questionnaire 仍有該欄位，型別為 string 並帶 `tcr-codetable-pending` 擴充 |

**待補的 38 欄**分三群，長表碼冊都有定義：
...
補這 38 欄的工作方式與已完成的 SSF 相同：轉錄官方編碼範圍到
