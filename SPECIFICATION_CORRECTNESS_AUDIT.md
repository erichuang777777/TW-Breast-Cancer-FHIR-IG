...
Profile／Extension 規格也採逐項核對，而不是只看 Publisher 總數。目前正式集合為 **48 個 StructureDefinition：35 Profile、13 Extension**；其中 4 個 TCR extension 是手寫 JSON，不能因不在 FSH 清單中而漏掉。`artifact-conformance-register.csv` 鎖定每個 artifact 的 parent、type、kind、scope 與合成範例，技術稽核為 48/48 pass；`profile-constraint-baseline.csv` 再逐列鎖定全部 **222 個 differential element**，包含 77 個 cardinality、106 個 Must Support、19 個 binding、55 個 type／profile target、29 個 fixed/pattern、2 個 slicing、0 個自訂 invariant，以及 18 個僅宣告／敘述列。每列保存完整 differential JSON 的 SHA-256；增刪元素或改變任一約束都會使發布 CI 失敗。這只證明交付審查的規格沒有漂移，不能把目前 **0/48** 的逐 artifact 臨床／FHIR reviewer 核准改寫成已通過，所以 RC-07 繼續 blocked。0 個自訂 invariant 也不是自動錯誤，但審查者必須確認跨欄位規則是否已由 slicing、binding、CQL 或其他明示機制完整表達。
...
全 IG 另有一份不可用「總數相同」取代的精確 resource manifest：**264 個 type/id** 必須逐項一致，其中 226 個 definitions、38 個 synthetic examples、224 個 canonical resources；264 項中 161 項由 FSH 生成、103 項為手寫 JSON。技術稽核要求生成的 ImplementationGuide 精確引用其餘 263 項，並驗證 example 分類、canonical、Library／Measure 與 NamingSystem 關係。CapabilityStatement 另鎖定為 `draft`、`experimental=true`、requirements-level 的 **13 種 resource type／20 個 supportedProfile** 精確集合；空集合、缺列、多列、重複、profile type 不符或改寫成可支援所有 Task 的宣稱都會失敗。這證明發布包沒有漏件或角色漂移，不證明每一資源的臨床內容正確。

完整引用圖也不能只靠 Publisher「沒有 unresolved warning」推定。`audit_fhir_reference_graph.py` 逐一驗證 **1042 個 edge**：669 個本地 URL link、336 個 FHIR `Reference.reference`、37 個外部 canonical；其中本地目標 200 個唯一 URL 均須存在且符合 ValueSet、CodeSystem、StructureDefinition、Library、Measure、Questionnaire 或 NamingSystem 的預期型別，Bundle fullUrl 亦須與內嵌 resource identity 相同。外部集合鎖成 19 個 URL，只能由 FHIR R4 `4.0.1` 與 TW Core `1.0.0` 兩個已固定 authority 解析。任何新增外部 canonical、斷裂 reference 或目標型別漂移都會使 RC-02 失敗。

版本語意缺口涵蓋全部 224 個 canonical resources，而非只有 Questionnaire。精確分類為：24 個明確使用 IG package version、1 個 Library 使用 CQL lifecycle version、98 個生成資源沒有 resource-level version 且尚未核准 package-context-only policy、101 個手寫 TCR canonical resources 全部使用 `4.0.1`。最後一組與 FHIR R4 版本相同，卻沒有 TCR 表單、手冊或碼表的權威 business-version 來源。FHIR R4 [`Questionnaire.version`](https://hl7.org/fhir/R4/questionnaire-definitions.html#Questionnaire.version) 明確將該欄位定義為作者管理的 business version；同類 canonical resources 亦不能把 `fhirVersion` 當成內容版本。四組政策目前 **0/4** 具名簽核，故 RC-08 business-version provenance gate 為 blocked。不得直接把 101 項改成推測年份或版號，也不得默認 98 項一定可只靠 package context。
...
上述要求已落成機器可讀登錄，而不再只是一段文字：[`source-traceability-register.csv`](mappings/publication/source-traceability-register.csv) 精確鎖定 52 個輸入 fact，目前完整骨架 52/52、19 維 source contract 完成 0/52、權威來源核准 0/52；追蹤 CQL 間接呼叫、Measure 參數與 task-layer 人工判定後，68 個 population／stratifier criteria 引用其中 **45 個 fact**。[`source-acquisition-priority.csv`](mappings/publication/source-acquisition-priority.csv) 再將全部 52 項機器分為 P0 42／P1 4／P2 6。為了讓院內可以直接派工，[`source-acquisition-work-packages.csv`](mappings/publication/source-acquisition-work-packages.csv) 將同一組 52 facts 分成 8 個角色工作包及 5 個批次：B0 直接結果阻擋 10、B1 其餘 cohort/rate 31、B2 provenance 1、B3 stratifier 4、B4 support 6；目前具名 owner 確認 0/52。優先級不會改變 52/52 都是 production publication 必要項。另由 [`measure-validation-evidence-register.csv`](mappings/publication/measure-validation-evidence-register.csv) 精確鎖定 20 個 Measure，並以 [`case-level-comparison-register.csv`](mappings/publication/case-level-comparison-register.csv) 驗證 exact case-by-expression／source-fact tuple set、normalizer/manifest hash 與完整 MeasureReport；目前逐案 manifest 0 列、獨立重算 0/20、完整期別 golden cohort 0/20。`audit_data_correctness_evidence.py` 禁止 secondary report 取代主來源，也禁止只有 `approved` 字樣、卻缺來源契約、reviewer、鎖版 hashes、完整逐案集合或零總差異證據的假通過；RC-01 另要求 52/52 accountable owner 有具名簽署證據。

FHIR 投影也不能只檢查 mapping 的 `target_path` 非空。`mapping-profile-projection-register.csv` 現將 34 個共同 fact 拆成精確 **55 個 target alternatives**，由 `audit_mapping_profile_projection.py` 對 Publisher 產生的完整 snapshot 逐列解析 Profile canonical、ElementDefinition id/path、choice datatype、Period child、slice 與單位註記。現況為 **49 個本地 Profile element 路徑可解析、1 個五輸入 derived rule、5 個 target alternatives unresolved**；結構受阻 fact 是 `CM-BC-024` 放療劑量、`CM-BC-032` 組織型態、`CM-BC-034` 轉院組織關係、`CM-BC-035` 放療療程覆蓋及 `CM-BC-037` 最後接觸時間。

更嚴格的資源語意檢查仍有 **5 個 semantic Profile gaps，影響 4 個 facts**：`CM-BC-006` pN 與 `CM-BC-007` pT 被投影至 stage-group shell；`CM-BC-018` 診斷性 core biopsy 被投影至只宣告 treatment scope 的 Procedure shell；`CM-BC-035` 已使用實際 `Procedure`／`MedicationAdministration`，但 event status 不能單獨證明 curative intent 或 course-level treatment semantics。`CM-BC-020／021／022／025` 已改用 MedicationAdministration medication、effective time 與 actual performer，並以 CQL 反例證明 MedicationRequest 醫囑本身不計入治療。這些判定依據本 IG 固定的 FHIR R4 4.0.1 ElementDefinition，而非報表欄名推測。`CM-BC-038` 另只有 EpisodeOfCare start，缺 episode-scoped new-diagnosis discriminator，不能完整建立 prior-year cohort。結構、語意與 coverage 缺口的聯集為 **9/34 blocked projection facts**。另有 ER、PR、Ki-67 三個 Quantity 路徑可解析，但 `%` 單位仍須來源契約與臨床驗證。RC-01 現要求 55/55 精確且語意適配的投影，unresolved path、semantic Profile gap、unit-policy-pending 與 blocked fact 全為 0；路徑可解析仍不能取代 52/52 原始來源證據。
...
這 15 條非完全對齊 criterion 現已逐項落入 [`criterion-resolution-decision-register.csv`](mappings/publication/criterion-resolution-decision-register.csv)，分成 10 個不可混淆的決策包。N1-HT、D2-SURGERY-FIRST、N4-ANTIHER2 已完成 MedicationAdministration 語意修正與 order-only 負向測試，因此回到 aligned-to-current-draft；X1-STAGING-DEATH 與 X3-STAGING 仍因 curative-intent／course linkage 未核准而維持 candidate-not-approved。每列固定目前差異、必須回答的問題、必要產出、權責簽署人及可接受證據；目前核准 **0/15**。`audit_criterion_resolution_decisions.py` 會與 live crosscheck 比對，拒絕漏列、新增但未納管的缺口、基線問題被改寫、pending 列偷填決定、無日期／證據 URI／SHA-256 的簽核，以及 QI-06 三列互相矛盾的變體決定。RC-07 不只要求 15/15 簽署，還要求 live crosscheck 的 15 項全數不再 non-aligned 且 production disposition 全數允許；所以 Measure 層泛稱簽核或單純填滿 15 列都不能掩蓋具體缺口。
...
RC-07 要求每個 Measure approval 有 `approve`、signer、組織／職稱、ISO 日期、證據 URI 與 64 位 SHA-256，Measure audit 的 `draft_definition_alignment` 必須是 `approved`，48 個 StructureDefinition 必須全部核准，且上述 15 個 criterion resolution 必須逐項有完整證據與有效簽署。因此不能用空泛簽名接受仍標為 `known-incomplete`、`candidate` 或 `contradiction` 的規格。
