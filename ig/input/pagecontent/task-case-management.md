...
`bc-qr-02` 執行時必須傳入 `Reporting Organization System Root`，其值為經院方治理確認的臺大醫療體系根 `Organization.id`。接收機構的 `Organization.partOf` 必須指向該根機構才計入體系內轉院；參數缺漏或只知道接收機構有任意上層機構時，一律不加計，避免高估分子。
