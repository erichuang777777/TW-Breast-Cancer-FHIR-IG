...
RC-07 另要求 `artifact-conformance-register.csv` 精確涵蓋全部 **47 個 StructureDefinition（34 Profile、13 Extension）**。`scripts/audit_artifact_conformance.py` 會把 FSH 產物與 4 個手寫 TCR extension 一起比對 parent canonical、FHIR type/kind、draft/experimental 狀態及合成範例使用證據；`scripts/audit_profile_constraints.py` 則把 47 個 artifact 的 **216 個 differential element** 與 `profile-constraint-baseline.csv` 逐列、逐欄及逐 SHA-256 比對。現行 facet 數為 cardinality 77、Must Support 100、binding 19、type／profile target 53、fixed/pattern 29、slicing 2、自訂 invariant 0、僅宣告／敘述 18；facet 可重疊，216 是受控元素數而非 facet 加總。人工核准還必須直接指向當次實際生成的 `StructureDefinition-*.json`，稽核會重算 SHA-256，禁止 artifact 或任一約束改變後沿用舊核准。現況技術一致性為 47/47、constraint baseline 216/216，但精確 artifact 核准為 0/47；Publisher 綠燈或 baseline 一致都不能替代這 47 項核准。所有 QBC、Measure、criterion 與 scope 核准也必須指向 repository 內可讀且 hash 相符的證據，詳見 [`APPROVAL_EVIDENCE_PROTOCOL.md`](APPROVAL_EVIDENCE_PROTOCOL.md)。
...
- `mapping-profile-projection-register.csv` 必須精確涵蓋 34 個共同 fact 的 **55 個 target alternatives**。每列鎖定來源 Profile 表達、target expression、Profile canonical、Publisher snapshot 的 ElementDefinition id/path、choice type、datatype child、unit 與阻擋原因。目前 49 列解析到本地 Profile element、1 列為五輸入 derived rule、5 列 unresolved；5 個結構受阻 fact 為 CM-BC-024／032／034／035／037。語意檢查另找出 12 個 semantic Profile gaps，影響 CM-BC-006／007／018／020／021／022／025／035：pN/pT 誤用 stage-group shell、診斷性切片誤用 treatment Procedure shell，以及把 MedicationRequest 醫囑、`authoredOn` 開立時間、預期 performer 或醫囑 status 當成實際治療證據。CM-BC-038 另只有 episode start，不能識別 prior-year new-diagnosis cohort。另 3 列 `%` Quantity 是 path-resolved／unit-policy-pending。結構、語意與 coverage 缺口的聯集為 13 個 blocked projection facts。正式門檻為 55/55 有精確且語意適配的 Profile/ElementDefinition 或核准 derivation，且 unresolved path、semantic Profile gap、unit-policy-pending、blocked fact 均為 0；路徑正確不代表來源與臨床語意正確。
...
- 68 條 criterion 中，50 條與目前草稿對齊；其餘 18 條已鎖定為 13 個決策包，核准目前為 0/18。新增的結果影響缺口包括：以 MedicationRequest 醫囑冒充實際給藥（N1-HT、N4-ANTIHER2）、以 `authoredOn` 冒充治療開始（D2-SURGERY-FIRST）、同一醫囑語意錯置傳遞至死亡前治療判斷（X1-STAGING-DEATH、X3-STAGING），以及 X1-NOSTAY-TRANSFER 未執行規格所寫的「無院內治療」條件。每列必須回答固定問題並附實作／資料契約／真值集／逐案差異等指定證據；QI-06 三列必須引用同一個不可矛盾的簽署決定。RC-07 要求 18/18 有效簽署、live crosscheck 非對齊數為 0，且這 18 項 production disposition 允許數為 18；簽名不能覆蓋仍存在的技術缺口。
