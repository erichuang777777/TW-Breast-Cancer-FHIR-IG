...
RC-07 另要求 `artifact-conformance-register.csv` 精確涵蓋全部 **48 個 StructureDefinition（35 Profile、13 Extension）**。`scripts/audit_artifact_conformance.py` 會把 FSH 產物與 4 個手寫 TCR extension 一起比對 parent canonical、FHIR type/kind、draft/experimental 狀態及合成範例使用證據；`scripts/audit_profile_constraints.py` 則把 48 個 artifact 的 **222 個 differential element** 與 `profile-constraint-baseline.csv` 逐列、逐欄及逐 SHA-256 比對。現行 facet 數為 cardinality 77、Must Support 106、binding 19、type／profile target 55、fixed/pattern 29、slicing 2、自訂 invariant 0、僅宣告／敘述 18；facet 可重疊，222 是受控元素數而非 facet 加總。人工核准還必須直接指向當次實際生成的 `StructureDefinition-*.json`，稽核會重算 SHA-256，禁止 artifact 或任一約束改變後沿用舊核准。現況技術一致性為 48/48、constraint baseline 222/222，但精確 artifact 核准為 0/48；Publisher 綠燈或 baseline 一致都不能替代這 48 項核准。所有 QBC、Measure、criterion 與 scope 核准也必須指向 repository 內可讀且 hash 相符的證據，詳見 [`APPROVAL_EVIDENCE_PROTOCOL.md`](APPROVAL_EVIDENCE_PROTOCOL.md)。
...
RC-02 現在另以 `fhir-resource-inventory.csv` 鎖定生成後完整集合：**264 個 FHIR resources、226 個 publication definitions、38 個 synthetic examples、224 個 canonical resources**；來源為 161 個 FSH 生成資源與 103 個手寫 JSON。`scripts/audit_fhir_resource_inventory.py` 要求 264 個 type/id 與 263 個 `ImplementationGuide.definition.resource` reference 精確一致，並核對 example／definition 標記、canonical 唯一性、20 Measure 到 CQL Library 的引用及 NamingSystem URI。CapabilityStatement 必須維持 `draft`、`experimental=true`、`kind=requirements`，且精確列出 **13 種 resource type 與 20 個 supportedProfile**；每個 profile 必須存在且 FHIR type 相符，REST 邊界必須明示不能據此宣稱支援所有 Task。

資源存在不代表引用正確，因此 RC-02 另鎖定完整 reference graph：共 **1042 個 edge**，包含 669 次本地 URL 連結、336 次 `Reference.reference` 與 37 次外部 canonical。669 次本地連結再分為 239 canonical fields、240 extension uses、153 CodeSystem uses、20 Bundle fullUrls、13 fixed extension URLs、2 NamingSystem uses、2 ConceptMap CodeSystem uses；200 個唯一的本地 URL target 全部可解析且目標 resource type 正確。336 次 FHIR Reference 包含 263 個 IG manifest references 與 73 個臨床／範例資源 references，全部可解析。37 次外部 canonical 只允許 19 個已審 URL，並由 FHIR R4 `4.0.1`（35 次）與 TW Core `1.0.0`（2 次）固定依賴解析。這證明引用圖技術閉合，不證明每一條臨床關係在真實資料上語意正確。

RC-08 進一步要求全部 224 個 canonical resources 的 business-version provenance，而不是只看 package version 或單一 Questionnaire。機器稽核把完整集合鎖成四個互斥群組：24 個明確使用 package version、1 個 CQL Library 使用 CQL lifecycle version、98 個生成資源目前只隱含於 package context、101 個手寫 TCR canonical resources 皆填入 `4.0.1`。後一數值與 FHIR R4 版本碰撞，但缺少 TCR 表單／手冊／碼表的權威版本證據；依 FHIR R4 的 [`Questionnaire.version`](https://hl7.org/fhir/R4/questionnaire-definitions.html#Questionnaire.version) 等 canonical resource version 定義，resource `version` 是內容的 business version，不是 `fhirVersion`。四組政策登錄於 `canonical-version-policy-register.csv`，目前 **0/4** 完成具名簽核，因此 gate blocked。這是語意／治理阻擋，不是 Publisher 結構錯誤；取得權威來源後須決定 98 項是否接受 package-context-only policy，並為 101 項填入或明確繼承真正的來源版本及證據，不得猜值。

## 數量與正確性門檻

### Mapping

- `source-traceability-register.csv` 必須精確涵蓋現行個管 mapping 的 **52 個 fact**（34 個共同 fact＋18 個 task-only fact）；追蹤 CQL 共用函式、Measure 參數與人工 adjudication 後，68 個 Measure criteria 目前引用其中 **45 個 fact**。其餘 7 個是報告、治理、Provenance 或尚未使用的候選輸入，仍須保留。每個 fact 必須恰有一個 authoritative／derived／pending 主列，另可增加報表的 `secondary-reconciliation` 列，但 secondary 列永遠不能使 RC-01 通過。每個 approved 主列還必須填滿並簽核 **19 個 source-contract dimensions**：data type、record grain、business key、join rule、cardinality、allowed-value domain、time semantics、timezone、precision/tolerance、unit、null、duplicate resolution、late-arriving update、invalid-value、extraction filter、transformation、FHIR absence、provenance 及 derivation inputs。空白、`TBD`、`N/A` 或沒有理由的 `not-applicable` 都不得通過。
...
- `mapping-profile-projection-register.csv` 必須精確涵蓋 34 個共同 fact 的 **55 個 target alternatives**。每列鎖定來源 Profile 表達、target expression、Profile canonical、Publisher snapshot 的 ElementDefinition id/path、choice type、datatype child、unit 與阻擋原因。目前 49 列解析到本地 Profile element、1 列為五輸入 derived rule、5 列 unresolved；5 個結構受阻 fact 為 CM-BC-024／032／034／035／037。語意檢查另找出 5 個 semantic Profile gaps，影響 CM-BC-006／007／018／035：pN/pT 誤用 stage-group shell、診斷性切片誤用 treatment Procedure shell，以及實際治療事件尚未建立 curative-intent／course linkage。全身性治療已改用 `MedicationAdministration.medication[x]`、`effective[x]` 與 actual performer；`MedicationRequest` 僅代表醫囑。CM-BC-038 另只有 episode start，不能識別 prior-year new-diagnosis cohort。另 3 列 `%` Quantity 是 path-resolved／unit-policy-pending。結構、語意與 coverage 缺口的聯集為 9 個 blocked projection facts。正式門檻為 55/55 有精確且語意適配的 Profile/ElementDefinition 或核准 derivation，且 unresolved path、semantic Profile gap、unit-policy-pending、blocked fact 均為 0；路徑正確不代表來源與臨床語意正確。
- 現況為 52/52 骨架存在、0/52 完成 19 維 source contract、0/52 取得完整權威來源與簽核；已知的 molecular subtype derived fact 已明列五個輸入。`audit_data_correctness_evidence.py` 會拒絕漏列、重複主列、未知 fact、過期名稱／indicator family、任一 contract dimension 缺漏、無解釋的 placeholder、derived input 未知／重複／自我依賴／循環依賴，以及缺證據卻自稱 approved 的列。
...
- Measure 規格核准目前為 0/20；每一項都必須保存具名 signer、組織／職稱、決定日期、repository-relative 證據路徑與被簽 artifact 的 SHA-256，稽核須能讀取檔案並重算出同一 hash，且 `draft_definition_alignment` 必須明確為 `approved`。`measure-specification-approval-audit.json` 還會要求 20/20 bundle 精確符合當次 FSH、CQL、生成 Measure、catalog、criteria、measure audit 與 approval context 七個 hash，並要求至少 208 個 Measure×criterion 正／負 truth assertions；格式見 [`MEASURE_SPECIFICATION_APPROVAL_PROTOCOL.md`](MEASURE_SPECIFICATION_APPROVAL_PROTOCOL.md)。只有簽名欄位、任意 64 碼字串或只有綠色 CQL 測試都不足以通過 RC-07。
- 68 條 criterion 中，53 條與目前草稿對齊；其餘 15 條已鎖定為 10 個決策包，核准目前為 0/15。N1-HT、D2-SURGERY-FIRST、N4-ANTIHER2 已用 MedicationAdministration 與 order-only 負向案例解除 FHIR 資源語意錯置；X1-STAGING-DEATH、X3-STAGING 仍待核准 curative-intent／course linkage，X1-NOSTAY-TRANSFER 仍未執行規格所寫的「無院內治療」條件。每列必須回答固定問題並附實作／資料契約／真值集／逐案差異等指定證據；QI-06 三列必須引用同一個不可矛盾的簽署決定。RC-07 要求 15/15 有效簽署、live crosscheck 非對齊數為 0，且這 15 項 production disposition 允許數為 15；簽名不能覆蓋仍存在的技術缺口。
